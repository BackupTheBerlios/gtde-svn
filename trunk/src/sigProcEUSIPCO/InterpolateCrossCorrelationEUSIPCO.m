function [CrossCorrelation, Derivative, Curvature] = InterpolateCrossCorrelationEUSIPCO(FirstSignal,SecondSignal,Delays,SamplingPeriod,MaxDelay)
...
%   see also PolynomialInterpolationCoefficients, PolynomialCoefficientsCrossCorrelation and performCCInterpolation
    ...
    [FirstPolynomialCoefficients] = PolynomialInterpolationCoefficientsEUSIPCO(FirstSignal,...
                                                                        SamplingPeriod);
    [SecondPolynomialCoefficients] = PolynomialInterpolationCoefficientsEUSIPCO(SecondSignal,...
                                                                         SamplingPeriod);
    
    %%% Compute the cross-correlation of the coefficients
    if nargin < 5
        [PCCC] = PolynomialCoefficientsCrossCorrelationEUSIPCO(FirstPolynomialCoefficients,...
    ...
    for dd = 1:numel(Delays),
        ...
        [CrossCorrelation(dd), Derivative(dd), Curvature(dd)] = CCInterpolationEUSIPCO(PCCC,...
                                                                Delays(dd),...
                                                                SamplingPeriod);
    end
