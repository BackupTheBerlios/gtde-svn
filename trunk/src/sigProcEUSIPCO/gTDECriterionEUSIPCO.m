function [J, GJ, HJ] = gTDECriterionEUSIPCO(TDEs,PCCC,microphones,samplingPeriod)

% gTDECriterion implements the criterion to optimize for geometric TDE
% 
%   J = gTDECriterion(TDEs,PCCC,microphones,samplingPeriod)
%   TDEs is the TDEs at which the criterion should be computed.
%   PCCC is the polynomial coefficients cross correlation (use to interpolate
%   the cross-correlation functions at the points computed from TDEs
%   microphones are the microphones' position
%   samplingPeriod is the samplingPeriod of the discrete signal
%      
...
% % %     =========================================
    ...
    for ss = 2:NMics,
        ...
        [r(index,:), rd(index,index,:), rdd(index,index,:)] = CCInterpolationEUSIPCO(PCCC{1,ss},-TDEs(index,:),samplingPeriod);
    end
    ...
    for mic1 = 2:NMics,
        % Autocorrelation
        R(mic1-1,mic1-1,:) = CCInterpolationEUSIPCO(PCCC{mic1,mic1},0,samplingPeriod);
        for mic2 = mic1+1:NMics
            % Crosscorrelation
            TDEmic1mic2 = TDEs(TDEis(1,mic2,NMics),:)-TDEs(TDEis(1,mic1,NMics),:);
            % The cross-correlation function should be interpolated at
            % -t_{mic1,mic2} not at t_{mic1,mic2}
            [R(mic1-1,mic2-1,:), RD(mic1-1,mic2-1,:), RDD(mic1-1,mic2-1,:)] = CCInterpolationEUSIPCO(PCCC{mic1,mic2},-TDEmic1mic2,samplingPeriod);
    ...
    RTilde(1,1,:) = CCInterpolationEUSIPCO(PCCC{1,1},0,samplingPeriod);
    RTilde(1,2:end,:) = r;
    RTilde(2:end,1,:) = r;
