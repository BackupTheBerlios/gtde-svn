...
def K2(p, q, tau, T):
    ...
    # Local Variables: tau, p, q, k2, r, T, k2dd, k2d
    # Function calls: nchoosek, K2, zeros, nargout, size
    #%K2   K2 function
    #%
    ...
    if nargout > 1.:
        ...
        for r in np.arange(0., (q-1.)+1):
            k2d = k2d+np.dot(nchoosek(q, r), (-tau)**(q-r-1.))*(matdiv(np.dot(r-q, T**(r+p+1.)-tau**(r+p+1.)), r+p+1.)+tau**(r+p+1.))
    ...
    if nargout > 2.:
        k2dd = np.zeros(matcompat.size(tau))
        #% The general formula works for r < q-1
        for r in np.arange(0., (q-2.)+1):
            k2dd = k2dd+np.dot(nchoosek(q, r), (-1.)**(q-r-2.))*(matdiv(np.dot(np.dot(1.+r-q, r-q), T**p*tau**(q-1.)*(T/tau)**(r+1.)-tau**(q+p-1.)), r+p+1.)+np.dot(r-p-2.*q, tau**(q+p-1.)))
            
        #% If both q and p are 0, do not add anything
        if q+p > 0.:
            #% Term for r = q-1
        k2dd = k2dd+np.dot(np.dot(q, q+p+1.), tau**(q+p-1.))
        #% Term for r = q
        k2dd = k2dd-np.dot(q+p, tau**(q+p-1.))
